class MathGraph {
  ...
  setupEventListeners() {
    ...
    ['xmin', 'xmax', 'ymin', 'ymax', 'color'].forEach(id => {
  ...
  setupMouseEvents() {
    ...
    // // Double click chuột trái để xóa tất cả điểm đã chọn
  ...
  findClickedPoint(clickX, clickY) {
    for (let i = 0; i < this.selectedPoints.length; i++) {
      ...
      // Tính khoảng cách từ điểm click đến điểm đã chọn
  ...
  updateViewportFromInputs() {
    this.viewport.xMin = parseFloat(document.getElementById('xmin').value) || -10;
    this.viewport.xMax = parseFloat(document.getElementById('xmax').value) || 10;
    this.viewport.yMin = parseFloat(document.getElementById('ymin').value) || -10;
    this.viewport.yMax = parseFloat(document.getElementById('ymax').value) || 10;
  }

  updateInputs() {
    document.getElementById('xmin').value = Math.round(this.viewport.xMin * 100) / 100;
    document.getElementById('xmax').value = Math.round(this.viewport.xMax * 100) / 100;
    document.getElementById('ymin').value = Math.round(this.viewport.yMin * 100) / 100;
    document.getElementById('ymax').value = Math.round(this.viewport.yMax * 100) / 100;
  }

  worldToScreen(x, y) {
    // Thêm padding logic 24px mỗi cạnh để không vẽ sát mép
    const padding = 24;
  ...
  drawAxes() {
    ...
    // Tính toán grid steps dựa trên viewport hiện tại
    ...
    for (let x = startX; x <= endX; x += xGridStep) {
      ...
      // Draw label - chỉ vẽ nếu đủ khoảng cách
  ...
  drawSelectedPoints() {
    ...
    this.selectedPoints.forEach((point, index) => {
      ...
      // Cập nhật tọa độ màn hình cho điểm
      point.screenX = sx;
      point.screenY = sy;

      // Chọn màu cho điểm (lặp lại nếu có nhiều điểm hơn số màu)
      const colorIndex = index % pointColors.length;
      const pointColor = pointColors[colorIndex];

      // Vẽ điểm được chọn (vòng tròn)
      this.ctx.save();
      ...
      // Xác định vị trí hiển thị nhãn (tránh che điểm và các nhãn khác)
      let labelX = sx + 15;
      let labelY = sy - 15;
      
      // Đảm bảo nhãn không vượt ra ngoài canvas
      ...
      // Lưu vị trí nhãn để tránh chồng chéo
      point.labelX = labelX;
      point.labelY = labelY;
  ...
  draw() {
    ...
    // Vẽ tất cả điểm được chọn (sau cùng để nằm trên cùng)
